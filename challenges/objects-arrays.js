...
const regex = /Uni*/;
for (let i = 0; i < graduates.length; i++) {
  if (regex.test(graduates[i].university)){
...
*/
const largerPopulation = zooAnimals.filter((index) => {
  return index.population < 5;
});
...
*/
const populationTotal = zooAnimals.reduce((runtotal, index) => {
  return runtotal + index.population;
}, 0);
